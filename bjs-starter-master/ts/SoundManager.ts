class SoundManager {
    ...
    public static soundCounter;

    public static mall: BABYLON.Sound;
    public static disco: BABYLON.Sound;

    public static currentSoundDialogue: BABYLON.Sound;
    ...
    public static loadSounds(assetManager: BABYLON.AssetsManager) {
    ...
    public static loadMusics(assetManager: BABYLON.AssetsManager) {
    ...
    public static soundReady() {
        SoundManager.soundCounter -= 1;
        if (SoundManager.soundCounter == 0) console.log("les sons sont prêt!");
    }

    public static playSound(pID) {
    ...
    public static playMusic(pName) {
        switch (pName) {
            case "mall":
                if (!SoundManager.mall.isPlaying) SoundManager.mall.play();
                break;
            case "disco":
                if (!SoundManager.disco.isPlaying) SoundManager.disco.play();
                break;
            default: console.log("play music "+ pName);
        }
    }

    public static stopMusic(pName) {
        switch (pName) {
            case "mall":
                if (SoundManager.mall.isPlaying) SoundManager.mall.stop();
                break;
            case "disco":
                if (SoundManager.disco.isPlaying) SoundManager.disco.stop();
                break;
